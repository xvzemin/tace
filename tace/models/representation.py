...
from .angular import AngularICTD, AngularGTO
from .radial import RadialBasis
from .rotate import init_edge_rot_mat
...
class TACEDescriptor(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.max_r_1 = max_r_1
        self.max_r_2 = max_r_2
        ...
        self.tc_type = inter.get('tc_type', 'SO(3)')
        if self.tc_type == 'SO(2)' or self.tc_type == 'O(2)':
            rotate_fn = torch.fx.symbolic_trace(
                lambda T1, T2: torch.einsum('bij, bj -> bi', T1, T2)
            )
            self.rotate_fn = opt_einsum_fx.optimize_einsums_full(
                model=rotate_fn,
                example_inputs=(
                    torch.randn([5, 3, 3]),
                    torch.randn([5, 3]),
        ...
        if max_r_1 < max(target_rank):
            raise ValueError(
                f"cfg.model.config.max_r_1 {max_r_1} should be greatet than"
                f"the tensor property you want to predict {max(target_property)}."
            )
    
        rank_of_node = list(range(max_r_1 + 1))
        rank_of_edge = list(range(max_r_2 + 1))
        ...
        for layer in range(num_layers):
            if layer == 0:
                rank_of_in.append([0])
                rank_of_hidden.append(rank_of_edge)
                rank_of_out.append(target_rank if layer == num_layers - 1 else rank_of_node)
            else:
                rank_of_in.append(rank_of_node)
                rank_of_hidden.append(rank_of_edge)
                rank_of_out.append(target_rank if layer == num_layers - 1 else rank_of_node)
        ...
        self.angular_embedding = nn.ModuleDict()
        for r in rank_of_edge:
            if angular_type == 'gto':
                self.angular_embedding[str(r)] = AngularGTO(r, norm)
            else:
                self.angular_embedding[str(r)] = AngularICTD(r, norm)
    ...
    def forward(self, data: Dict[str, Tensor], graph: Graph) -> Dict[str, Any]:
        ...
        rot_mat_inv = None
        if self.tc_type == 'SO(2)' or self.tc_type == 'O(2)':
            rot_mat = init_edge_rot_mat(normed_edge_vector)
            rot_mat_inv = rot_mat.transpose(-1, -2)
            normed_edge_vector = self.rotate_fn(rot_mat, normed_edge_vector)
        for r_2_str, module in self.angular_embedding.items():
            r_2 = int(r_2_str)
            edge_attrs[r_2] = module(normed_edge_vector).unsqueeze(1)
