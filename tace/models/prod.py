...
from .paths import satisfy, generate_prod_paths, parse_einsum_expr
from .ictd import ICT_decomposition
from .kernel.broadcast import TargetBroadcastTC
from .kernel.triton import TargetTritonTC
...
class SelfContraction(torch.nn.Module):
    ...
    def __init__(
        ...
        tensor_product_only = prod.get("tensor_product_only", False)
        ...
        self.paths_list_list, self.exprs_list_list = generate_prod_paths(
            max_r_in, rank_of_out, correlation, allow_nosym, restriction, tensor_product_only,
        )
        ...
        for v in range(self.correlation - 1):
            ...
            for (r_1, r_2, r_o), expr in zip(
                ...
                if self.kernel == 'einsum':
                    ...
                    ctrs.append(TargetTritonTC(expr))
    ...
    def forward(
        ...
        TMP = torch.jit.annotate(
            ...
            {0: {int(r): [node_feats[r]] for r in node_feats}},
        )

        for r_o in range(self.max_r_in + 1):
            self.Qs[r_o] = self.Qs[r_o].to(device=device, dtype=dtype) 
        ...
        for v, ctrs in enumerate(self.ctrs):
            ...
            for idx, ctr in enumerate(ctrs):
                ...
                if self.decomposition:
                    ...
                    tmp = torch.bmm(
                        tmp.reshape(P, B * C, -1), self.Qs[r_o].repeat(P, 1, 1)
                    ).reshape((P, B, C) + REST)
