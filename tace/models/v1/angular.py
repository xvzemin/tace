...
from cartnn.o3 import ICTD, Irreps
...
class LegacyCartesianHarmonics2(torch.nn.Module):
    def __init__(self, lmax: int, norm: bool = True, traceless: bool = True) -> None:
        ...
        for l in range(self.lmax+1):
            PS, DS, CS, SS = ICTD(l)
            self.register_buffer(f"Q{l}", DS[0].to(torch.get_default_dtype()))
            del PS, DS, CS, SS
    ...
    def Q(self, l: int):
        return dict(self.named_buffers())[f"Q{l}"]
...
def check_recursively_detrace():
    ...
    for n in range(1, 10):
        T = symmetric_traceless_outer_product(v, n)
...
def check_ict_decomposition():
    ...
    for n in range(1, 6):
        ...
        T = LegacyCartesianHarmonics1(n)(v)
        _bool = check_all_traces_zero(T)
        print(f"n={n}: All traces zero? {_bool}; shape: {T.shape}")
        assert _bool
        check_ctr_to_unity(T, v)
...
def check_equivalent():
    ...
    v = v / torch.norm(v, keepdim=True, dim=-1)
    for n in range(1, 6):
        T1 = symmetric_traceless_outer_product(v, n)
        T2 = symmetric_outer_product(v, n)
        T2 = LegacyCartesianHarmonics1(n)(v)
