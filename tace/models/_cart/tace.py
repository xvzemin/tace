...
class cartTACE(torch.nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.target_irreps = co3.Irreps(target_irreps).regroup()
    ...
    def _select_tensor_fidelity(
        ...
        """Select node fidelities from a flattened ``ir_mul`` readout."""
        features = features.reshape(
            features.size(0), irrep_dim, self.num_fidelities
        ).transpose(1, 2)
        return features[num_atoms_arange, node_fidelity]
    ...
    def readout_fn(
            ...
            if from_representation["decouple_node_feats1"] is None:
                ...
                for ii, direct_forces_readout in enumerate(self.direct_forces_readouts):
                    if not self.use_alllayer:
                        ii = -1
                    d_f_list.append(
                        self._select_tensor_fidelity(
                            direct_forces_readout(descriptors[ii], node_fidelity),
                            3,
                            num_atoms_arange,
                            node_fidelity,
                        )
        ...
        if "direct_dipole" in self.target_property:
            ...
            for ii, dipole_readout in enumerate(self.dipole_readouts):
                ...
                d_list.append(
                    self._select_tensor_fidelity(
                        dipole_readout(descriptors[ii], node_fidelity),
                        3,
                        num_atoms_arange,
                        node_fidelity,
                    )
        ...
        if "direct_polarizability" in self.target_property:
            ...
            for ii, (polarizability_readout0, polarizability_readout2) in enumerate(
                ...
                alpha2_list.append(
                    self._select_tensor_fidelity(
                        polarizability_readout2(descriptors[ii], node_fidelity),
                        9,
                        num_atoms_arange,
                        node_fidelity,
                    )
        ...
        if (
            ...
            if from_representation["decouple_node_feats1"] is None:
                ...
                for ii, (direct_virials_readout0, direct_virials_readout2) in enumerate(
                    ...
                    d_v2_list.append(
                        self._select_tensor_fidelity(
                            direct_virials_readout2(descriptors[ii]),
                            9,
                            num_atoms_arange,
                            node_fidelity,
                        )
        ...
        if self.target_irreps.count("0e"):
        ...
        if hasattr(self, "dens_noise_readouts"):
            if from_representation["decouple_node_feats2"] is not None:
                ...
                for ii, dens_noise_readout in enumerate(self.dens_noise_readouts):
                    ...
                    noise_list.append(
                        self._select_tensor_fidelity(
                            dens_noise_readout(descriptors[ii], node_fidelity),
                            3,
                            num_atoms_arange,
                            node_fidelity,
                        )
