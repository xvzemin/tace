...
def generate_combinations(
    ...
    tensor_product_only: bool = False,
):
    combs = []
...
def generate_prod_paths(
    max_rank_of_in, rank_of_out, correlation, allow_nosym, restriction, tensor_product_only,
):
    paths_list_list = []
    exprs_list_list = []
    current_ranks = set(range(max_rank_of_in + 1))

    for layer in range(correlation - 1):
        paths_list = []
        exprs_list = []
        next_ranks = set()
    ...
    for layer in reversed(range(correlation - 1)):
        filtered_paths = []
        filtered_exprs = []
        next_valid_ranks = set()

        for (r_1, r_2, r_o), exprs in zip(
            paths_list_list[layer], exprs_list_list[layer]
        ):
        ...
        paths_list_list[layer] = filtered_paths
        exprs_list_list[layer] = filtered_exprs
