...
from .so2_ctr import SO2_Contraction
...
class Interaction(torch.nn.Module):
    ...
    def __init__(
        ...
        if self.tc_type == 'SO(3)' or self.tc_type == 'O(3)':
            self.tc = Contraction(
        ...
        elif self.tc_type == 'SO(2)' or self.tc_type == 'O(2)':
            self.tc = SO2_Contraction(
                num_channel=num_channel,
                num_channel_hidden=num_channel_hidden,
                max_r_in=max_r_in,
                max_r_out=max_r_out,
            ...
            raise ValueError(f"Unsupported tc_type: {self.tc_type}. Expected 'O(3)' or 'O(2)'.")
        ...
        if weight == 'max':
            ...
            for r in range(max_r_out + 1):
                _, Qs = ICT_decomposition(r)
                num_weights = len(Qs)
                self.num_weights.append(num_weights)
                self.Qs.append(Qs[0].to(dtype=torch.get_default_dtype()))
        elif weight == 'all':
            self.decomposition = True
            ...
            for r in range(max_r_out + 1):
                ...
                self.num_weights.append(num_weights)
                self.Qs.append(
                    torch.stack(Qs, dim=0).to(dtype=torch.get_default_dtype())
                )
    ...
    def forward(
        ...
        cutoff: Tensor,
        graph: Graph,
        rot_mat: Optional[Tensor],
        rot_mat_inv: Optional[Tensor],
        ...
        if hasattr(self, 'density_normalizer'):
            ...
            density = scatter_sum(
                source=edge_density, index=edge_index[1], dim=0, dim_size=node_attrs_lmp.shape[0]
            )
            # if lmp:
        ...
        m_ji = self.tc(node_feats, edge_attrs, edge_feats, cutoff, rot_mat, rot_mat_inv)
