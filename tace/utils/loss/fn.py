...
@register_loss
def mse_dipole(pred: Dict[str, Tensor], label: Dict[str, Tensor]) -> Tensor:
    total_weight = (label.entropy * label.dipole_weight).unsqueeze(-1)
    return torch.mean(torch.square((label["dipole"] - pred["dipole"])) * total_weight)
...
@register_loss
def mse_polarizability_per_atom(
    pred: Dict[str, Tensor], label: Dict[str, Tensor]
) -> Tensor:
    total_weight = (
        (label.entropy * label.polarizability_weight).unsqueeze(-1).unsqueeze(-1)
    )
    num_atoms = (label.ptr[1:] - label.ptr[:-1]).unsqueeze(-1).unsqueeze(-1)
    return torch.mean(
        torch.square((label["polarizability"] - pred["polarizability"]) / num_atoms)
        * total_weight
    )
...
def mse_direct_virials_per_atom(pred: Dict[str, Tensor], label: Dict[str, Tensor]) -> Tensor:
    ...
    num_atoms = (label.ptr[1:] - label.ptr[:-1]).view(-1, 1, 1)
    return torch.mean(
        torch.square(pred["direct_virials"] - label["direct_virials"])
        / num_atoms
        * total_weight.unsqueeze(-1).unsqueeze(-1)
    )