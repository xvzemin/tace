...
class TensorProduct(torch.nn.Module):
    ...
    irreps_in1 : IrrepsLike
        Cartesian representation of the first input.
    irreps_in2 : IrrepsLike
        Cartesian representation of the second input.
    irreps_out : IrrepsLike
        Requested Cartesian output representation.
    channels_in1 : int
        Number of channels carried by every first-input irrep copy.
    channels_in2 : int, optional
        Number of second-input channels. Defaults to one for ``"u1u"`` and to
        ``channels_in1`` otherwise.
    channels_out : int, optional
        Number of output channels. Defaults to ``channels_in1``.
    project : bool
        If ``True``, project every output onto its symmetric traceless
        subspace. If ``False``, return the ambient Cartesian result so linear
        aggregation can precede :meth:`project_output`.
    path_mode : {"u1u", "uuu", "uvw"}, optional
        Channel contraction. ``"u1u"`` uses a single second-input channel,
        ``"uuu"`` couples matched channels, and ``"uvw"`` performs a dense
        input-input-output channel contraction.
    internal_weights : bool, optional
        If ``True``, store trainable weights in the module. Otherwise weights
        must be passed to :meth:`forward`.
    path_norm : bool, optional
        Normalize multiple paths into one output by their inverse square-root
        count.
    path : sequence of tuple of int, optional
        Explicit ``(output_index, input1_index, input2_index)`` paths over
        expanded irrep copies. All legal paths are generated by default.

    Notes
    -----
    Inputs and outputs use ``(..., irreps.dim, channels)``. Delta contractions
    generate even rank-difference paths and Levi-Civita contractions generate
    odd rank-difference paths.
    """
    ...
    def project_output(self, input: torch.Tensor) -> torch.Tensor:
        """Project a dense output onto every requested irreducible subspace.

        Parameters
        ----------
        input : torch.Tensor
            Ambient Cartesian tensor with shape
            ``(..., irreps_out.dim, channels_out)``.

        Returns
        -------
        torch.Tensor
            Projected tensor with the same shape as ``input``.
        """
    ...
    def forward(
        ...
        """Evaluate all Cartesian tensor-product paths.

        Parameters
        ----------
        input1 : torch.Tensor
            First input with shape
            ``(..., irreps_in1.dim, channels_in1)``.
        input2 : torch.Tensor
            Second input with shape
            ``(..., irreps_in2.dim, channels_in2)``.
        weight : torch.Tensor, optional
            External weights with trailing shape ``weight_shape``. Leading
            dimensions broadcast with both inputs. Omit when internal weights
            are enabled.

        Returns
        -------
        torch.Tensor
            Output with shape ``(..., irreps_out.dim, channels_out)`` over the
