...
def test_shared_magnetic_basis_matches_separate_bases():
    ...
    shared = MagneticChebyshevBasis(num_basis=9, include_constant=True)(x)
    interaction = MagneticChebyshevBasis(num_basis=8, include_constant=False)(x)
    one_body = MagneticChebyshevBasis(num_basis=8, include_constant=True)(x)

    torch.testing.assert_close(shared[..., 1:], interaction)
    torch.testing.assert_close(shared[..., :-1], one_body)
...
def test_one_body_magmoms_is_enabled_for_magnetic_input():
    ...
    assert model.representation.magnetic_radial_basis.num_basis == 9
...
def test_one_body_magmoms_energy_bypasses_scale_shift():
    ...
    one_body_magmoms_basis = torch.randn(num_nodes, 8)
